...
var p{projects}, >= 0, <= 1;
/* fund left at then end of year */
var s{0..card(years)}, >= 0;

maximize z: sum{project in projects} p[project] * returns[project];

subject to
fund_limit{year in years } :
	sum{project in projects} outlays[year, project] * p[project] + s[year] - s[year-1] <= funds[year];
	
s_limit_first_year: s[0] = 0;
